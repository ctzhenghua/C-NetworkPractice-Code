...
\chapter{TCP/IP 基础篇}    
	...
	\section{传输层 相关知识：TCP/UDP }
		\subsection{TCP}
			...
			\subsubsection{连接建立3次握手}
			
			\subsubsection{连接终止4次握手}
			
			\subsubsection{TCP 如何保证可靠性}
				\begin{itemize}
					\item 当TCP发出一个段后，它启动一个定时器，等待目的端确认收到这个报文段。如果不能及时收到一个确认，将重发这个报文段。
					
					\item 当TCP收到发自TCP连接另一端的数据，它将发送一个确认。这个确认不是立即发送，通常将推迟几分之一秒。
					
					\item TCP将保持它首部和数据的校验和，目的是检验数据在传输过程中的任何变化。如果收到段的校验和有差错，TCP将丢弃这个报文段并且不确认（导致对方超时重传） 
					
					\item TCP 承载于IP数据报来传输，而IP数据报的到达可能会失序，因此TCP报文段的到达也可能会失序，TCP将对收到的数据进行重新排序。
					
					\item IP数据报会发生重复，TCP的接收端必须丢弃重复的数据。
					
					\item TCP还能提供流量控制。TCP连接的每一方都有一定大小的缓冲空间。
				\end{itemize}
		...
		\subsection{4次握手:终止链接}
			...
			\subsubsection{Notice}
				...
				\subparagraph{TIME\_WAIT 存在的理由}
					\begin{itemize}
						\item  虽然按道理，四个报文都发送完毕，我们可以直接进入\verb|CLOSE|状态了，但是\textbf{我们必须假象网络是不可靠的}，有可以最后一个\verb|ACK|丢失。所以\verb|TIME_WAIT|状态就是用来\textbf{重发可能丢失的ACK报文}
						
						\item 允许老的重复分节在网络中消逝，这样就不会在新建在原来的IP端口上收到失效的过期数据，这也是\verb|TIME_WAIT|存在2MSL的原因
						为什么是两倍的MSL,因为这样可以保证发送的字节组方向能够消失，而且还可以保证ack方向的确认报文也可以消失。
					\end{itemize}
	\section{套接字选项}
		\subsection{相关函数}
			\begin{itemize}
