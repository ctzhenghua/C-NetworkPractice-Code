...
\end{lstlisting}	
		\subsection{核心实现}
			\subparagraph{backtrace}栈回溯，保存各个栈帧的地址
				
				\verb|backtrace(buffer, len) |returns a backtrace for the calling program, in  the  array
				pointed  to  by  \verb|buffer|.
				
				return a number of void* stack pointer frame.
				
			\subparagraph{backtrace\_symbol} 根据栈帧地址，转成相应的函数符号，return a 指针的指针
			\subparagraph{abi::\_\_cxa\_demangle}:将c++ 编译后的名字改回原名称,实现参考上节
				\begin{lstlisting}
	void Exception::fillStackTrace()
	{
		...
		if (strings)
		{
			...
			{
				// TODO demangle funcion name with abi::__cxa_demangle, 可以利用此将名称改回
